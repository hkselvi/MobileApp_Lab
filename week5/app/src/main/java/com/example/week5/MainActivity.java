...
public class MainActivity extends AppCompatActivity {
    ...
    class CellListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            if (!isValidMove(row, col)){
                Toast.makeText(MainActivity.this,
                         "This cell is already occupied!", Toast.LENGTH_LONG).show();
                return;
            }
            if (player1Turn){
            ...
            if (gameEnded(row,col) == -1){
                player1Turn =! player1Turn;  //comparing if the term is true then it is gonna be false!!
        ...
        public int gameEnded(int row, int col){
            ...
            return -1; //the game will continue
        }

        public boolean isValidMove(int row, int col){
            return board[row][col] == 0;  // it means if the cell is empty
        }
    ...
    protected void onRestoreInstanceState(@NonNull Bundle savedInstanceState) {
        ...
        for (int i = 0; i < 9; i++) {
            board[i/3][i%3] = boardSingle[i]; //conversion
        }
